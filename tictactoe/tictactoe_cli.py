import sqlite3
import random
from tictactoe_questions import questions

# Connect to the SQLite database
conn = sqlite3.connect('tictactoe_scores.db')
cursor = conn.cursor()

# Create a table to store tictactoe scores if it doesn't exist
cursor.execute('''CREATE TABLE IF NOT EXISTS tictactoe_scores
             (username text, tictactoe_score int)''')

def print_highest_score(player_name):
    # Query the database for the highest score of the specified player
    cursor.execute("SELECT MAX(tictactoe_score) FROM tictactoe_scores WHERE username=?", (player_name,))
...
for category in questions['tictactoe categories']:
    print(f"{category_num}. {questions['tictactoe categories'][category_num - 1]}")
    category_num += 1
...
while category_selection not in [1, 2, 3, 4]:
    if category_selection.isdigit() and 1 <= int(category_selection) <= 4:
        ...
        selected_category = questions['tictactoe categories'][category_selection-1]
    else:
...
cursor.execute("INSERT INTO tictactoe_scores VALUES (?, ?)", (player_name, score))
conn.commit()
...
#     # Execute an SQL statement to delete all records from the "tictactoe_scores" table
#     cursor.execute("DELETE FROM tictactoe_scores")
#     conn.commit()
