...
# Create a table to store quiz scores if it doesn't exist
c.execute('''CREATE TABLE IF NOT EXISTS scores
             (name text, score int)''')

def print_highest_score(player_name):
    # Query the database for the highest score of the specified player
    c.execute("SELECT MAX(score) FROM scores WHERE name=?", (player_name,))
...
#     # Execute an SQL statement to delete all records from the "scores" table
